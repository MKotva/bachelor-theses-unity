...
namespace Assets.Core.GameEditor.Compile
{
    public static class Patterns
    {
        ...
        // language=regex
        private static readonly string value = $"({booleanValue}|{numericValue})";
        // language=regex
        ...
        // language=regex
        private static readonly string typeKeyWords = @"(^|\s)(num|bool)(\s|$)";
        // language=regex
        ...
        //// language=regex
        //private static readonly string funcArgument = $"({variable}|{value}|\\(|\\)|\\ |{boolOperator}|{aritmeticOperator})+";
        // language=regex
        ...
        //// language=regex
        //private static readonly string assignValuePattern = $"^((?<assignValue_type>{type}) )?(?<assignValue_variable>{variable})\\ ?=\\ ?(?<assignValue_expression>{expression})$";
        // language=regex
        private static readonly string assignValuePattern = $"^\\s*((?<assignValue_type>{type})\\s*)?(?<assignValue_variable>{variable})\\s*=\\s*(?<assignValue_expression>{expression})$";
        // language=regex
        ...
        // language=regex
        private static readonly string valueTypePattern = $"((?<booleanValue>{booleanValue})|(?<numericValue>{numericValue})|(?<stringValue>{stringValue}))";
        ...
        static Patterns()
        {
            ...
            linePattern.Append($"|(?<assignValue>{assignValuePattern})");
            ...
            AssingRegex = new Regex(assignValuePattern, regexOptions);
        }
    }
    //public static class Patterns
    //{
    ...
    //    //// language=regex
    //    private static readonly string function = $"((?<![^\\s+*\\/\\-\\(\\)])\\w+(\\((\\s*{funcArgument}\\s*,)*(\\s*{funcArgument}\\s*)\\)|\\(\\s*\\)(?![^\\s+*\\/-])))";
    //    // language=regex
    //    private static readonly string variable = @"((?<![^\s+*\/\-\(\)])(\w*[a-zA-Z]\w*\.)*(\w*[a-zA-Z]\w*)(?![^\s+*\/-]))";
    //    // language=regex
    //    private static readonly string booleanValue = "(true|false)";
    ...
    //    private static readonly string keyWords = @"(^|\s)(if|while|elseif|else|fi|end|while)(\s|$)";
    //    // language=regex
    //    private static readonly string expression = $"({variable}|{value}|\\(|\\)|\\ |{boolOperator}|{aritmeticOperator}|{function})+";
    //    // language=regex
    //    private static readonly string funcArgument = $"({variable}|{value}|\\(|\\)|\\ |{boolOperator}|{aritmeticOperator})+";
    //    // language=regex
    //    private static readonly string ifPattern = $"^if (?<if_expression>{expression})$";
    ...
    //        StringBuilder expressionTerminalPattern = new();
    //        expressionTerminalPattern.Append("(");
    //        expressionTerminalPattern.Append($"(?<boolOperator>{boolOperator})");
    //        expressionTerminalPattern.Append($"|(?<aritmeticOperator>{aritmeticOperator})");
    //        expressionTerminalPattern.Append($"|(?<value>{value})");
