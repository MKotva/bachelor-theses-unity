...
namespace Assets.Core.SimpleCompiler.Compilation.CodeBase
{
    public class AssignLine : ICodeLine
    {
        ...
        /// <summary>
        /// Assigns the result of expression to variable(global,local,property)
        /// </summary>
        ...
        public void Execute()
        {
            ...
            else
            {
                ...
                {
                    context.GlobalVariables[variableName] = ParseAssign(context.GlobalVariables[variableName], value);
                }
                else if (!context.LocalVariables.ContainsKey(variableName))
                {
                    if(op.OperatorText != "=") 
                    {
